...
export default function ComputationPage({ params }: { params: { unitId: string } }) {
  ...
  const validityText = (() => {
    ...
    return `VALID UNTIL: ${last.toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" })} (subject to unit availability)`;
  })();
        ...
        <div className="bg-blue-900 text-white p-6 rounded-t-xl">
          ...
          <p className="mt-2 text-red-300 font-semibold">{validityText}</p>
              ...
              <div className="p-4 border-b text-sm grid sm:grid-cols-3 gap-2">
                <div>
                  Turnover date: <span className="font-semibold text-blue-900">{selectedUnit.RFODate}</span>
                </div>
                <div>
                  Building | Unit type: <span className="font-semibold text-blue-900">{selectedUnit.BuildingUnit} | {selectedUnit.Type}</span>
                </div>
                <div>
                  Total area: <span className="font-semibold text-blue-900">{selectedUnit.GrossAreaSQM}</span>
                </div>
              </div>
            ...
            <div className="flex gap-3 mt-2">
              <button onClick={downloadPNG} className="px-4 py-2 bg-blue-600 text-white rounded-lg">
                Download PNG
              </button>
              <button onClick={downloadPDF} className="px-4 py-2 bg-green-600 text-white rounded-lg">
                Download PDF
              </button>
              <button onClick={downloadExcel} className="px-4 py-2 bg-yellow-500 text-white rounded-lg">
                Download Excel
              </button>
            </div>
          </div>
