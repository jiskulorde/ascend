// src/components/projects/AGPPageClient.tsx
"use client";
...
export default function AGPPageClient({ isAuthenticated }: { isAuthenticated: boolean }) {
  ...
  useEffect(() => {
    ...
    // including ListPrice. Fixed to read the real keys the endpoint returns;
    // no formula/logic change, just correct field access.
    async function loadAuthenticated() {
      ...
      const normalized: UnitRow[] = json.data.map((p: any) => ({
        Property: p.Property || "",
        BuildingUnit: p.BuildingUnit || "",
        Tower: p.Tower || "",
        Floor: p.Floor || "",
        Status: p.Status || "",
        Type: p.Type || "",
        GrossAreaSQM: Number(p.GrossAreaSQM || 0),
        Amenities: p.Amenities || "",
        Facing: p.Facing || "",
        RFODate: p.RFODate || "",
        ListPrice: Number(p.ListPrice || 0),
        PerSQM: Number(p.PerSQM || 0),
      }));
      setUnits(normalized);
    }
    ...
    // left blank since nothing on this page reads it for the preview path,
    // and the preview endpoint doesn't expose it anyway (no BuildingUnit,
    // no exact per-sqm price, no tower/property codes).
    async function loadPreview() {
      const merged: UnitRow[] = [];
      let page = 1;
      let hasMore = true;

      while (!cancelled && hasMore && page <= 3) {
        const res = await fetch(`/api/availability/preview?project=AGP&page=${page}`);
        const json = await res.json();
